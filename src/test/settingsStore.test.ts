...
test("SettingsStore repairs smoke sentinel base URLs back to provider defaults", async () => {
  await withTempDir(async (userDataPath) => {
    ...
    const current = store.get();
    assert.equal(current.baseUrl, "https://openrouter.ai/api/v1");
    assert.equal(current.ollamaBaseUrl, "http://localhost:11434/v1");

    const persistedPath = join(userDataPath, "cipher-workspace", "cipher-workspace-settings.json");
    const persisted = JSON.parse(await readFile(persistedPath, "utf8")) as { baseUrl?: string; ollamaBaseUrl?: string };
    assert.equal(persisted.baseUrl, "https://openrouter.ai/api/v1");
    assert.equal(persisted.ollamaBaseUrl, "http://localhost:11434/v1");
  });
});
